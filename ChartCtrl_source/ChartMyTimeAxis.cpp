...
CChartMyTimeAxis::CChartMyTimeAxis()
...
void CChartMyTimeAxis::SetTickIncrement(bool bAuto, double newIncrement)
{
	m_bAutoTicks = bAuto;
	if (!m_bAutoTicks)
	{
...
double CChartMyTimeAxis::GetFirstTickValue() const
{
...
bool CChartMyTimeAxis::GetNextTickValue(double dCurrentTick, double& dNextTick) const
{
...
long CChartMyTimeAxis::ValueToScreenDiscrete(double dValue) const
{
...
long CChartMyTimeAxis::GetTickPos(double TickVal) const
{
...
TChartString CChartMyTimeAxis::GetTickLabel(double TickValue) const
{
	TChartStringStream ssLabel;
	int min = TickValue / 60;
	int second = TickValue - 60 * min;
	char time[8] = {0};
	snprintf(time, 8, "%02d:%02d", min, second);

	ssLabel << fixed << setprecision(m_uDecCount) << time;
	return ssLabel.str();
}

void CChartMyTimeAxis::RefreshTickIncrement()
{
	if (!m_bAutoTicks)
		return;
...
void CChartMyTimeAxis::RefreshFirstTick()
{
	if (m_dTickIncrement != 0)
	{
		if (m_MinValue == 0)
